...
from src.vector_store.qdrant_client import init_qdrant_client, get_embedding_model, search_qdrant
import traceback
...
def get_chat_engine():
    ...
    try:
        ...
        llm = get_llm()
        if not llm:
            st.error("❌ LLM模型加载失败 - 请检查模型文件是否存在")
            return None
        else:
        ...
        try:
            ...
            st.error(f"❌ Qdrant客户端初始化失败: {str(e)}")
        ...
        st.success("✅ 问答引擎初始化完成")
        return qa_chain
        
    except Exception as e:
        st.error(f"❌ 问答引擎初始化失败: {str(e)}")
        st.error(f"详细错误: {traceback.format_exc()}")
        return None